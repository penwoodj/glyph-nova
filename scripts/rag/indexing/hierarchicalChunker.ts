 ...
 *
 * Creates parent-child chunk relationships to balance precision (child chunks)
 * with context (parent chunks). Child chunks are small (200-300 chars) for
 * precise retrieval, while parent chunks are large (1000-1500 chars) for
 * comprehensive context.
 *
 * Process:
 * 1. Generate child chunks (200-300 chars) from document
 * 2. Group children into parent chunks (1000-1500 chars)
 * 3. Store parent-child relationships
 * 4. When child retrieved, include parent for context
 *
...
export class HierarchicalChunker {
  private childChunkSize: number; // 200-300 chars
  private parentChunkSize: number; // 1000-1500 chars
  private childOverlap: number; // Overlap for child chunks
  private parentOverlap: number; // Overlap for parent chunks

  constructor(
    childChunkSize: number = 250,
    parentChunkSize: number = 1200,
    childOverlap: number = 30,
   ...
   */
  private createChildChunks(content: string, sourceFile: string, sourcePath: string): HierarchicalChunk[] {
  ...
  private createParentChunks(
    ...
    let currentParentChildren: HierarchicalChunk[] = [];
   ...
   */
  private linkChildrenToParents(childChunks: HierarchicalChunk[], parentChunks: HierarchicalChunk[]): void {
   ...
   */
  private getChunkId(chunk: HierarchicalChunk): string {
   ...
   */
  getParentChunk(childChunk: HierarchicalChunk, allChunks: HierarchicalChunk[]): HierarchicalChunk | undefined {
   ...
   */
  getChildChunks(parentChunk: HierarchicalChunk, allChunks: HierarchicalChunk[]): HierarchicalChunk[] {
