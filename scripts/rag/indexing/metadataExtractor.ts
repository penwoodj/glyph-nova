 ...
 * Metadata Extractor
 *
 * Extracts rich metadata from documents and chunks to enable abstraction-aware filtering.
 * Metadata includes: document type, section, abstraction level, keywords, topics, timestamp, author, version.
 *
 * Process:
 * 1. Extract document type from file extension/path
 * 2. Extract section information (markdown headers)
 * 3. Classify abstraction level (high/medium/low) using LLM
 * 4. Extract keywords and topics
 * 5. Store timestamp, author, version
 *
...
export interface EnrichedMetadata {
  ...
  documentType?: string; // e.g., 'markdown', 'typescript', 'python', 'text'
...
export class MetadataExtractor {
  ...
  private useLLMClassification: boolean;

  constructor(
    ollamaModel: string = 'llama2',
    ollamaUrl: string = 'http://localhost:11434',
    useLLMClassification: boolean = true
  ) {
   ...
   */
  private extractSection(documentContent: string, startIndex: number, endIndex: number): string | undefined {
   ...
   */
  private async classifyAbstractionLevel(text: string): Promise<'high' | 'medium' | 'low'> {
...
Respond with only one word: high, medium, or low.`;
   ...
   */
  private classifyAbstractionLevelHeuristic(text: string): 'high' | 'medium' | 'low' {
