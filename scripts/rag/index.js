 ...
 */
async function indexDocuments(paths, useJson = false, enableWatch = false) {
    console.log(`\n=== Indexing Documents ===\n`);
 ...
 */
async function reindexSingleFile(filePath, useJson, storeDir) {
    const fileCollector = new FileCollector();
 ...
 */
async function startFileWatcher(filePaths, useJson) {
    const storeDir = join(process.cwd(), '.rag-store');
 ...
 */
async function queryDocuments(query, useJson, expandQueries = false, useReranking = false) {
    console.log(`\n=== Querying Documents ===\n`);
