...
import { HierarchicalChunker, HierarchicalChunk } from './indexing/hierarchicalChunker.js';
...
async function indexDocuments(
  ...
  useHierarchicalChunking: boolean = false
) {
  console.log(`\n=== Indexing Documents ===\n`);
  ...
  const allChunks: Array<{ text: string; sourceFile?: string; sourcePath?: string; startIndex: number; endIndex: number; chunkIndex: number; parentId?: string; childIds?: string[]; isParent?: boolean; isChild?: boolean }> = [];
  const fileContents: Map<string, string> = new Map(); // Store file contents for metadata extraction
  ...
  for (const file of files) {
    ...
    if (useHierarchicalChunking && hierarchicalChunker) {
      ...
      chunks = hierarchicalChunks.map((c: HierarchicalChunk) => ({
        text: c.text,
  ...
  if (enrichMetadata && metadataExtractor) {
    ...
    const enrichedChunks = await Promise.all(
      allChunks.map(async (chunk, i) => {
        ...
        const enrichedMetadata = await metadataExtractor.extractMetadata(
          chunk.text,
          chunk.sourcePath || '',
          chunk.startIndex,
          chunk.endIndex,
 ...
 */
async function reindexSingleFile(
  filePath: string,
  useJson: boolean,
  storeDir: string,
  useSemanticChunking: boolean = false,
